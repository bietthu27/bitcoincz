...
CzBCZTracker::CzBCZTracker(std::string strWalletFile)
{
...
CzBCZTracker::~CzBCZTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBCZTracker::Init()
{
...
bool CzBCZTracker::Archive(CMintMeta& meta)
{
...
bool CzBCZTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBCZTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBCZTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBCZTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBCZTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBCZTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBCZTracker::GetSerialHashes()
{
...
CAmount CzBCZTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBCZTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBCZTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBCZTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBCZTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBCZTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBCZTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBCZTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBCZTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBCZTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBCZTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBCZWallet* zBCZWallet)
{
    bool iszBCZWalletInitialized = (NULL != zBCZWallet);
    CMintMeta meta;
    ...
    if (! iszBCZWalletInitialized)
        zBCZWallet = new CzBCZWallet(strWalletFile);
    meta.isSeedCorrect = zBCZWallet->CheckSeed(dMint);
    if (! iszBCZWalletInitialized)
        delete zBCZWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBCZTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBCZTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBCZTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBCZTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBCZTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBCZTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBCZWallet* zBCZWallet = new CzBCZWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zBCZWallet);
        }
        delete zBCZWallet;
        LogPrint("zero", "%s: added %d dzbcz from DB\n", __func__, listDeterministicDB.size());
    }
