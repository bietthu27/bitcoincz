...
#include "ui_zbczcontroldialog.h"
...
std::set<std::string> ZBczControlDialog::setSelectedMints;
std::set<CMintMeta> ZBczControlDialog::setMints;

bool CZBczControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZBczControlDialog::COLUMN_DENOMINATION || column == ZBczControlDialog::COLUMN_VERSION || column == ZBczControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZBczControlDialog::ZBczControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBczControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBczControlDialog::~ZBczControlDialog()
{
    delete ui;
}

void ZBczControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBczControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZBczControlWidgetItem* itemDenom(new CZBczControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZBczControlWidgetItem *itemMint = new CZBczControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zbczTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbczTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                strReason = strprintf("Needs %d more confirmations", Params().Zerocoin_MintRequiredConfirmations() - nConfirmations);
                            ...
                            strReason = "Your wallet is locked. Impossible to precompute or spend zBCZ.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBCZ seed used to mint this zBCZ is not the same as currently hold in the wallet";
            else if (model->getEncryptionStatus() == WalletModel::EncryptionStatus::Locked)
                strReason = "Your wallet is locked. Impossible to precompute or spend zBCZ.";
            else
...
void ZBczControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBczControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBcz_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBczControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBczControlDialog::GetSelectedMints()
{
...
void ZBczControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
