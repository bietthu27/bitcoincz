                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zBCZ ought to be enough for anybody." - Bill Gates, 2017
    ui->zBCZpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBCZSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBCZpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBCZ_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBCZ is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zBCZ, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBCZ...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBCZ in ") +
...
void PrivacyDialog::on_pushButtonSpendzBCZ_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBCZ is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zBCZ, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zBCZ
        sendzBCZ();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBCZ
    sendzBCZ();
}

void PrivacyDialog::on_pushButtonZBczControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZBczControlDialog* zBczControl = new ZBczControlDialog(this);
    zBczControl->setModel(walletModel);
    zBczControl->exec();
}

void PrivacyDialog::setZBczControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBczSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBCZ()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Bcz Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBCZpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBCZ is requested
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBCZ</b>";
    ...
    // use mints from zBCZ selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBczControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZBczControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zBCZ was spent successfully update the addressbook with the label
    ...
    // Clear zbcz selector in case it was used
    ZBczControlDialog::setSelectedMints.clear();
    ui->labelzBczSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBCZ Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Bcz, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBCZ Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zbczTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBCZ </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBCZ "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBCZ "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zBCZ "));
    ui->labelzBCZAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, bczAvailableBalance2, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zBCZ </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zBCZ </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zBCZ </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzBCZSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzBCZ->isEnabled();
    bool fMaintenanceMode = GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE);

    if(fMaintenanceMode) {
        ui->labelOverviewInMaintenance->setVisible(true); }
    else {
        ui->labelOverviewInMaintenance->setVisible(false); }


    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zBCZ
        ui->pushButtonMintzBCZ->setEnabled(false);
        ui->pushButtonMintzBCZ->setToolTip(tr("zBCZ is currently disabled due to maintenance."));

        // Spend zBCZ
        ui->pushButtonSpendzBCZ->setEnabled(false);
        ui->pushButtonSpendzBCZ->setToolTip(tr("zBCZ is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zBCZ
        ui->pushButtonMintzBCZ->setEnabled(true);
        ui->pushButtonMintzBCZ->setToolTip(tr("PrivacyDialog", "Enter an amount of BCZ to convert to zBCZ", 0));

        // Spend zBCZ
        ui->pushButtonSpendzBCZ->setEnabled(true);
        ui->pushButtonSpendzBCZ->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
