...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zbczspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not BCZ at the same time
...
enum ZerocoinSpendStatus {
    ...
    ZBCZ_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
...
public:
    ...
    string GetUniqueWalletBackupName(bool fzbczAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzBCZTracker> zbczTracker;
    ...
    {
        zwalletMain = zwallet;
        zbczTracker = std::unique_ptr<CzBCZTracker>(new CzBCZTracker(strWalletFile));
    }

    CzBCZWallet* getZWallet() { return zwalletMain; }

    bool isZeromintEnabled()
    {
        if (fEnableZeromint){
            LogPrintf("Zeromint started\n");}
        if (fEnableAutoConvert){
            LogPrintf("AutoConvert started\n");}
        return fEnableZeromint || fEnableAutoConvert;
    }

    void setZBczAutoBackups(bool fEnabled)
    {
