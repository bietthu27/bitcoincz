...
{
    return (unsigned short)(GetArg("-port", 29500));
}
...
CNode* ConnectNode(CAddress addrConnect, const char* pszDest, bool obfuScationMaster)
{
    ...
    if (pszDest ? ConnectSocketByName(addrConnect, hSocket, pszDest, 29500, nConnectTimeout, &proxyConnectionFailed) :
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "BCZ " + FormatFullVersion();
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(seed.host.c_str(), vIPs)) {
                for (CNetAddr& ip : vIPs) {
                    int nOneDay = 24 * 3600;
                    CAddress addr = CAddress(CService(ip, 29500));
...
void ThreadOpenConnections()
{
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (addr.GetPort() != 29500 && nTries < 50)
                continue;
