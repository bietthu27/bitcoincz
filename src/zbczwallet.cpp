...
CzBCZWallet::CzBCZWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbcz seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBCZSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBCZSeed_deprecated()) {
                    LogPrintf("%s: Updated zBCZ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbcz seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbcz wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBCZWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
...
void CzBCZWallet::Lock()
{
    seedMaster = 0;
}

void CzBCZWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBCZWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
bool CzBCZWallet::LoadMintPoolFromDB()
{
...
void CzBCZWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBCZWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBCZWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbczTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBCZWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    // Add to zbczTracker which also adds to database
    pwalletMain->zbczTracker->Add(dMint, true);
...
void CzBCZWallet::SeedToZBCZ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBCZWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBCZWallet::GenerateDeterministicZBCZ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBCZWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBCZWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBCZWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint))
    {
    uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
