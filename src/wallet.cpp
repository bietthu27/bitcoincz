...
#include "zbczwallet.h"
#include "zbcztracker.h"
...
bool CWallet::AbandonTransaction(const uint256& hashTx)
{
    ...
    while (!todo.empty()) {
        ...
        if (currentconfirm == 0/* && !wtx.isAbandoned()*/) {
            ...
            wtx.nIndex = -1;
            //wtx.setAbandoned();//xxxx
            wtx.MarkDirty();
            //walletdb.WriteTx(hash, wtx);//xxxx
...
CAmount CWalletTx::GetUnlockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && vout[i].nValue == GetSporkValue(SPORK_15_MN_MODE_X) * COIN) continue; // do not count MN-like outputs //xxxx
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        else if (fMasterNode && vout[i].nValue == GetSporkValue(SPORK_15_MN_MODE_X) * COIN) {//xxxx
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex && nTimeFirstKey && (pindex->GetBlockTime() < (nTimeFirstKey - 7200)) && pindex->nHeight <= zerostart)
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to read zbcz
            if (fCheckZBCZ && pindex->nHeight >= zerostart) {
...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    if (fMatureOnly) {
        ...
        CAmount nBalance = 0;
        vector<CMintMeta> vMints = zbczTracker->GetMints(true);
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        set<CMintMeta> setMints = zbczTracker->ListMints(true, true, true);
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fMasterNode && pcoin->vout[i].nValue == GetSporkValue(SPORK_15_MN_MODE_X) * COIN);//xxxx
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT10000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if (found && fMasterNode) found = pcoin->vout[i].nValue != GetSporkValue(SPORK_15_MN_MODE_X) * COIN; // do not use Hot MN funds//xxxx
                } else if (nCoinType == ONLY_10000) {
                    found = pcoin->vout[i].nValue == GetSporkValue(SPORK_15_MN_MODE_X) * COIN;//xxxx
                } else {
                    found = true;
                }
                if (!found) continue;
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount, bool fPrecompute)
{
    ...
    if (GetBoolArg("-bczstake", true) && !fPrecompute) {
    ...
    //ZBCZ
    if (GetBoolArg("-zerostake", true) || (fPrecompute && IsSporkActive(SPORK_17_ZEROCOIN_STAKE) && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))) {
        //Only update ZBCZ set once per update interval
        bool fUpdate = false;
        ...
        set<CMintMeta> setMints = zbczTracker->ListMints(true, true, fUpdate);
        for (auto meta : setMints) {
            ...
            if (meta.nHeight < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                std::unique_ptr<CZBczStake> input(new CZBczStake(meta.denom, meta.hashStake));
                listInputs.emplace_back(std::move(input));
            }
        }
    }
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZBCZBalance = GetZerocoinBalance(false);
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal BCZ.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal BCZ.");
                    } else {
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(stakeInput.get(), nBits, block.GetBlockTime(), nTxNewTime, hashProofOfStake)) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZBCZ());
            ...
            //Mark mints as spent
            if (stakeInput->IsZBCZ()) {
                CZBczStake* z = (CZBczStake*)stakeInput.get();
...
void CWallet::CreateAutoMintTransaction(const CAmount& nMintAmount, CCoinControl* coinControl)
{
    if (nMintAmount > 0){
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zBCZ. Current percentage of zBCZ: %lf%%\n",
...
void CWallet::AutoZeromint()
{
    ...
    // zBCZ are integers > 0, so we can't mint 10% of 9 BCZ
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zBCZ\n", nBalance);
        return;
    }

    // Percentage of zBCZ we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zBCZ (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zBCZ amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zBCZ amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zBCZ We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough BCZ to mint this denomination
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    // Sign if these are bcz outputs - NOTE that zBCZ outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    if (!spend.Verify(accumulator)) {
        receipt.SetStatus(_("The transaction did not verify"), ZBCZ_BAD_SERIALIZATION);
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent zBCZ
        receipt.SetStatus(_("The coin spend has been used"), ZBCZ_SPENT_USED_ZBCZ);
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(zbczTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = zbczTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = zbczTracker->GetSpendCache(meta.hashStake);
            ...
            if (!GenerateAccumulatorWitness(coinWitness, mapAccumulators, pindexCheckpoint)) {
                receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                                  ZBCZ_FAILED_ACCUMULATOR_INITIALIZATION);
            ...
            libzerocoin::ZerocoinParams *paramsCoin = Params().Zerocoin_Params(false);
            ...
            if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (!mint.GetKeyPair(key))
                    return error("%s: failed to set zBCZ privkey mint version=%d", __func__, nVersion);
            ...
            try {
                ...
                if (!CheckCoinSpend(spend, accumulator, receipt)) {
                    receipt.SetStatus(_("CoinSpend: failed check"), ZBCZ_SPEND_ERROR);
                    return error("%s : %s", __func__, receipt.GetStatusMessage());
                }
                ...
                receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZBCZ_INVALID_WITNESS);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
        }
        break;
    ...
    if (nLockAttempts == 100) {
        ...
        receipt.SetStatus(_("could not get lock on cs_spendcache"), ZBCZ_TXMINT_GENERAL);
        return false;
    }
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zBCZ transaction
    vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        setMints = zbczTracker->ListMints(true, true, true); // need to find mints to spend
        if(setMints.empty()) {
...
string CWallet::GetUniqueWalletBackupName(bool fzbczAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzbczAuto ? "-autozbczbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZBczBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozbczbackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozbczbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozbczbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zbczbackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zbczbackuppath", ""));
    ...
    while (true) {
        ...
        for (std::unique_ptr <CStakeInput>& stakeInput : listInputs) {
...

            {
                ...
                CoinWitnessData* witnessData = zbczTracker->GetSpendCache(serialHash);
