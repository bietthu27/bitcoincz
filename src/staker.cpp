...
std::pair<int, std::pair<uint256, uint256> > pCheckpointCache2;
CBlockTemplate* CreateNewPOSBlock(const CScript& scriptPubKeyIn, CWallet* pwallet)
{
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zbczs that have been in mempool long
                    //and higher priority to zbczs that are large in value
            ...
            if (tx.IsZerocoinSpend()) {
                ...
                for (const CTxIn& txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        ...
                        if (libzerocoin::ExtractVersionFromSerial(spend.getCoinSerialNumber()) < libzerocoin::PrivateCoin::PUBKEY_VERSION)
                            fDoubleSerial = true;
        ...
        LogPrintf("CreateNewPOSBlock(): total size %u\n", nBlockSize);
        ...
        if (nHeight >= pCheckpointCache2.first || pCheckpointCache2.second.first != hashBlockLastAccumulated) {
            //For the period before v2 activation, zBCZ will be disabled and previous block's checkpoint is all that will be needed
            pCheckpointCache2.second.second = pindexPrev->nAccumulatorCheckpoint;
            if (pindexPrev->nHeight + 1 >= zerostart) {
                ...
                if (!CalculateAccumulatorCheckpoint(nHeight, nCheckpoint, mapAccumulators)) {
                    ...
                    pCheckpointCache2.first = nHeight + (10 - (nHeight % 10));

                    // the block hash of the last block used in the accumulator checkpoint calc. This will handle reorg situations.
                    pCheckpointCache2.second.first = hashBlockLastAccumulated;
                    pCheckpointCache2.second.second = nCheckpoint;
                }
            }
        }

        pblock->nAccumulatorCheckpoint = pCheckpointCache2.second.second;
...
CBlockTemplate* CreateNewPOSBlockWithKey(CReserveKey& reservekey, CWallet* pwallet)
{
    ...
    return CreateNewPOSBlock(scriptPubKey, pwallet);
}
...
bool ProcessBlockFound2(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake())
            pwalletMain->zbczTracker->RemovePending(pblock->vtx[1].GetHash());
...
void POSMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (fStake_BCZ) {
            ...
            while (vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || (pwallet->GetBalance() > 0 && nReserveBalance >= pwallet->GetBalance()) || !masternodeSync.IsBlockchainSynced())
            {
        ...
        unique_ptr<CBlockTemplate> pblocktemplate(CreateNewPOSBlockWithKey(reservekey, pwallet));
        if (!pblocktemplate.get())
            continue;

        CBlock* pblock = &pblocktemplate->block;
        IncrementExtraNonce2(pblock, pindexPrev, nExtraNonce);
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zBCZ with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    continue;
                }

                //Sign block with the zBCZ key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("POSMiner(): Signing new block with zBCZ key failed \n");
                    continue;
                }
            } else if (!SignBlock(*pblock, *pwallet)) {
                LogPrintf("POSMiner(): Signing new block with UTXO key failed \n");
                continue;
            }

            LogPrintf("POSMiner : proof-of-stake block was signed %s \n", pblock->GetHash().ToString().c_str());
