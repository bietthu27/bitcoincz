...
{
    if (true) {
        //accumulators need to use v2 params
...
{
    if (pindex->nHeight < zerostart || fVerifyingBlocks)
        return true;
...
bool GetAccumulatorValue(int& nHeight, const libzerocoin::CoinDenomination denom, CBigNum& bnAccValue)
{
    ...
    //Every situation should use this method
...
map<CoinDenomination, int> GetMintMaturityHeight()
{
    ...
    while (pindex && pindex->nHeight > zerostart) {
