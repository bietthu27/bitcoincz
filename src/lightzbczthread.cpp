...
/****** Thread ********/
void CLightWorker::ThreadLightZBCZSimplified() {
    RenameThread("bcz-light-thread");
    ...
    while (true) {
        try {
            ...
            LogPrintf("%s pop work for %s \n\n", "bcz-light-thread", genWit.toString());
            ...
            if (!pIndex) {
                ...
                if (blockHeight >= zerostart) {
                    ...
                    try {
                        ...
                        LogPrintStr(std::string("ThreadLightZBCZSimplified: ") + e.message + "\n");
                    ...
                    if (!res) {
                        ...
                        ss << heightStop;
                        if (genWit.getPfrom()) {
                            LogPrintf("%s pushing message to %s \n", "bcz-light-thread", genWit.getPfrom()->addrName);
            ...
            PrintExceptionContinue(&e, "lightzbczthread");
            break;
        }
    }
...
void CLightWorker::rejectWork(CGenWit& wit, int blockHeight, uint32_t errorNumber) {
    if (wit.getStartingHeight() == blockHeight){
        LogPrintf("%s rejecting work %s , error code: %s\n", "bcz-light-thread", wit.toString(), errorNumber);
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
