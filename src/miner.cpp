        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to bcz that have been in mempool long
                    //and higher priority to bcz that are large in value
...
bool fGenerate_BCZ = false;

void POWMiner(CWallet* pwallet)
{
    LogPrintf("POWMiner starting in 60 secs\n");
    MilliSleep(60000);
    LogPrintf("POWMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("POWMiner");
    ...
    while (true){
        ...
        unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey, pwallet));
        if (!pblocktemplate.get())
            continue;
        ...
        LogPrintf("Running POWMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBCZ(bool fGenerate_BCZ, CWallet* pwallet, int nThreads)
{
    ...
    if (nThreads == 0 || !fGenerate_BCZ)
        return;

    minerThreads = new boost::thread_group();
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ThreadPOWMiner, pwallet));
}
