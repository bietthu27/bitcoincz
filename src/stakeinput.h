...
public:
    ...
    virtual bool IsZBCZ() = 0;
    virtual CDataStream GetUniqueness() = 0;
    virtual uint256 GetSerialHash() const = 0;
};


// zBCZStake can take two forms
// 1) the stake candidate, which is a zcmint that is attempted to be staked
// 2) a staked zbcz, which is a zcspend that has successfully staked
class CZBczStake : public CStakeInput
{
private:
    uint32_t nChecksum;
    bool fMint;
...
public:
    explicit CZBczStake(libzerocoin::CoinDenomination denom, const uint256& hashSerial)
    {
        this->denom = denom;
    ...
    explicit CZBczStake(const libzerocoin::CoinSpend& spend);
    ...
    bool IsZBCZ() override { return true; }
...
public:
    ...
    bool CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut = 0) override;
    bool CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal) override;
    bool IsZBCZ() override { return false; }
